export const propertyData = [
  {
    id: 1,
    image: "/images/property/1.jpg",
    title: "10765 Hillshire Ave, Baton Rouge, LA 70810, USA",
  },
  {
    id: 2,
    image: "/images/property/2.jpg",
    title: "59345 STONEWALL DR, Plaquemine, LA 70764, USA",
  },
  {
    id: 3,
    image: "/images/property/3.jpg",
    title: "3723 SANDBAR DR, Addis, LA 70710, USA",
  },
  {
    id: 4,
    image: "/images/property/4.jpg",
    title: "Lot 21 ROYAL OAK DR, Prairieville, LA 70769, USA",
  },
  {
    id: 5,
    image: "/images/property/5.jpg",
    title: "710 BOYD DR, Unit #1102, Baton Rouge, LA 70808, USA",
  },
  {
    id: 6,
    image: "/images/property/6.jpg",
    title: "5133 MCLAIN WAY, Baton Rouge, LA 70809, USA",
  },
  {
    id: 7,
    image: "/images/property/7.jpg",
    title: "1574 Sharlo Ave, Baton Rouge, LA 70820, USA",
  },
  {
    id: 8,
    image: "/images/property/8.jpg",
    title: "2528 BOCAGE LAKE DR, Baton Rouge, LA 70809, USA",
  },
  {
    id: 9,
    image: "/images/property/9.jpg",
    title: "1533 NICHOLSON DR, Baton Rouge, LA 70802, USA",
  },
];

export const clientData = [
  {
    image: "/images/client/01.jpg",
    name: "Brian Carpenter",
    title: "Real Estate Investor",
    desc: '"The Royal Peak made property investment easy and transparent. The platform is user-friendly and had no hidden fees. Highly recommend it!"',
  },
  {
    image: "/images/client/02.jpg",
    name: "Jessica Day",
    title: "First-Time Homebuyer",
    desc: "I was thrilled with my homebuying experience. The Royal Peak made the entire process seamless and stress-free. Exceptional service from start to finish!",
  },
  {
    image: "/images/client/03.jpg",
    name: "Jennifer Smith",
    title: "Homeowner",
    desc: '"Finding my dream home was a breeze. The Royal Peak\'s platform is intuitive, offering all the information I needed. Appreciate the support!"',
  },
  {
    image: "/images/client/04.jpg",
    name: "David Johnson",
    title: "Property Seller",
    desc: '"Selling my property through The Royal Peak was a smooth and stress-free experience. The platform\'s reach ensured quick visibility."',
  },
  {
    image: "/images/client/05.jpg",
    name: "Sophia Williams",
    title: "Landlord",
    desc: '"The Royal Peak made it easy to rent out my property. Their detailed listings and support team helped me find tenants swiftly and hassle-free."',
  },
  {
    image: "/images/client/06.jpg",
    name: "Michael Brown",
    title: "Investor",
    desc: '"As an investor, finding lucrative properties is crucial. The Royal Peak\'s extensive listings and market data have been invaluable in making decisions."',
  },
];

export const brokerData = [
  {
    image: "/images/client/04.jpg",
    name: "Jack John",
    title: "Broker",
  },
  {
    image: "/images/client/05.jpg",
    name: "Krista John",
    title: "Broker",
  },
  {
    image: "/images/client/06.jpg",
    name: "Roger Jackson",
    title: "Broker",
  },
  {
    image: "/images/client/07.jpg",
    name: "Johnny English",
    title: "Broker",
  },
];

export const blogData = [
  {
    id: 1,
    image: "/images/property/1.jpg",
    tag: "Townter",
    title: "Skills That You Can Learn In The Real Estate Market",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "13th Sep 2023",
  },
  {
    id: 2,
    image: "/images/property/2.jpg",
    tag: "Townter",
    title: "Learn The Truth About Real Estate Industry",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "29th Nov 2023",
  },
  {
    id: 3,
    image: "/images/property/3.jpg",
    tag: "Townter",
    title: "10 Quick Tips About Business Development",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "29th Dec 2023",
  },
  {
    id: 4,
    image: "/images/property/4.jpg",
    tag: "Townter",
    title: "14 Common Misconceptions About Business Development",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "13th March 2023",
  },
  {
    id: 5,
    image: "/images/property/5.jpg",
    tag: "Townter",
    title: "10 Things Your Competitors Can Teach You About Real Estate",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "5th May 2023",
  },
  {
    id: 6,
    image: "/images/property/6.jpg",
    tag: "Townter",
    title: "Why We Love Real Estate",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "19th June 2023",
  },
  {
    id: 7,
    image: "/images/property/7.jpg",
    tag: "Townter",
    title: "110 Quick Tips About Real Estate",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "20th June 2023",
  },
  {
    id: 8,
    image: "/images/property/8.jpg",
    tag: "Townter",
    title: "15 Best Blogs To Follow About Real Estate",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "31st Aug 2023",
  },
  {
    id: 9,
    image: "/images/property/9.jpg",
    tag: "Townter",
    title: "Using Banner Stands To Increase Trade Show Traffic",
    desc: "The most well-known dummy text is the 'Lorem Ipsum', in the 16th century.",
    date: "1st Sep 2023",
  },
];
